...
public class ContactSelectActivity extends UI implements View.OnClickListener, android.support.v7.widget.SearchView.OnQueryTextListener {
    ...
    public static void startActivityForResult(Context context, Option option, int requestCode,String [] selectedUids) {
        Intent intent = new Intent();
        intent.putExtra(EXTRA_DATA, option);
        Bundle b=new Bundle();
        b.putStringArray("selectedUids",selectedUids);
        intent.putExtras(b);
        intent.setClass(context, ContactSelectActivity.class);
        ((Activity) context).startActivityForResult(intent, requestCode);
    }
    ...
    private void initContactSelectArea() {
        ...
        String[] array=b.getStringArray("selectedUids");
        if(array!=null)
        {
            if(selectedUids==null)
                selectedUids=new ArrayList<>();
            for(String accid:array)
            {
                selectedUids.add(accid);
            }
        }
